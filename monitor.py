...
import random
import string
from config import decoys, decoy_dir, trusted_processes, monitor_interval_sec
...
def handle_threat(proc, path, event_type):
    ...
    if proc:
        try:
            ...
            if proc_name.lower() in trusted_processes:
    ...
    log_event(timestamp, os.path.basename(path), event_type, pid, ppid, exe, cmdline, status, score)
